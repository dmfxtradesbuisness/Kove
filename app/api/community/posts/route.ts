...
export async function GET(request: NextRequest) {
  try {
    ...
    // Parallel enrichment
    const [likesRes, bookmarksRes, reactionsRes, myReactionsRes, profilesRes] = await Promise.all([
      postIds.length > 0
      ...
      postIds.length > 0
        ...
        : Promise.resolve({ data: [] }),
      userIds.length > 0
        ? createAdminClient().from('community_profiles').select('user_id, display_name, avatar_url').in('user_id', userIds)
        : Promise.resolve({ data: [] }),
    ])
